...
fn sigmaskRestore(origmask: *const std.c.sigset_t) void {
    ...
    _ = pthread_sigmask(linux.SIG.SETMASK, origmask, &discard);
}
...
const pthread_mutex_t = extern struct { data: [if (@sizeOf(c_ulong) == 8) 40 else 24]u8 align(@alignOf(usize)) = @splat(0) };
const pthread_cond_t = extern struct { data: [48]u8 align(@alignOf(usize)) = @splat(0) };
const pthread_rwlock_t = extern struct { data: [if (@sizeOf(c_ulong) == 8) 56 else 32]u8 align(@alignOf(usize)) = @splat(0) };
...
extern "c" fn pthread_rwlock_init(rwlock: *pthread_rwlock_t, attr: ?*const anyopaque) c_int;
...
extern "c" fn pthread_sigmask(how: c_int, set: ?*const std.c.sigset_t, oldset: ?*std.c.sigset_t) c_int;
extern "c" fn pthread_rwlock_rdlock(rwlock: *pthread_rwlock_t) c_int;
extern "c" fn pthread_rwlock_wrlock(rwlock: *pthread_rwlock_t) c_int;
extern "c" fn pthread_rwlock_tryrdlock(rwlock: *pthread_rwlock_t) c_int;
extern "c" fn pthread_rwlock_unlock(rwlock: *pthread_rwlock_t) c_int;
...
extern "c" fn pthread_attr_setstacksize(attr: *pthread_attr_t, stacksize: usize) c_int;
extern "c" fn pthread_attr_setguardsize(attr: *pthread_attr_t, guardsize: usize) c_int;
...
fn getQueue(fd: c_int, need: c_int) ?*AioQueue {
    ...
    _ = pthread_rwlock_rdlock(&maplock);
    var q: ?*AioQueue = null;
    if (map) |m| {
        if (m[a]) |ma| {
            if (ma[b]) |mab| {
    ...
    if (q == null and need != 0) {
        _ = pthread_rwlock_unlock(&maplock);
        if (fcntl(fd, linux.F.GETFD) < 0) return null;
        var allmask: std.c.sigset_t = undefined;
        _ = std.c.sigfillset(&allmask);
        masked = true;
        _ = pthread_sigmask(linux.SIG.BLOCK, &allmask, &origmask);
        _ = pthread_rwlock_wrlock(&maplock);
    ...
    if (q) |queue| _ = pthread_mutex_lock(&queue.lock);
    _ = pthread_rwlock_unlock(&maplock);
    if (masked) sigmaskRestore(&origmask);
    return q;
}
...
fn unrefQueue(q: *AioQueue) void {
    if (q.ref > 1) {
        q.ref -= 1;
        _ = pthread_mutex_unlock(&q.lock);
        return;
    }

    _ = pthread_mutex_unlock(&q.lock);
    _ = pthread_rwlock_wrlock(&maplock);
    _ = pthread_mutex_lock(&q.lock);
    if (q.ref == 1) {
        const fd = q.fd;
        const ufd: u31 = @intCast(fd);
        const a: u7 = @intCast(ufd >> 24);
...
fn notifySignal(signo: c_int, val: sigval_t) void {
    ...
    _ = linux.syscall3(
        .rt_sigqueueinfo,
        c_lib.syscallArg(linux.getpid()),
        c_lib.syscallArg(signo),
        @intFromPtr(&si),
    );
}
...
fn cleanup(ctx: ?*anyopaque) callconv(.c) void {
    ...
    _ = pthread_cond_broadcast(&q.cond);
    ...
    if (sev.sigev_notify == SIGEV_THREAD) {
        ...
        _ = pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, null);
...
fn ioThreadFunc(ctx: ?*anyopaque) callconv(.c) ?*anyopaque {
    ...
    if (op != LIO_READ and (op != LIO_WRITE or q.append != 0)) {
        while (true) {
            ...
            if (p != null) {
                _ = pthread_cond_wait(&q.cond, &q.lock);
            }
        }
    }
...
fn submit(cb: *aiocb, op: c_int) c_int {
    var ret: c_int = 0;
    var a: pthread_attr_t = undefined;
    var origmask: std.c.sigset_t = undefined;
    ...
    if (cb.aio_sigevent.sigev_notify == SIGEV_THREAD) {
        if (cb.aio_sigevent.__sev_fields.__sev_thread.sigev_notify_attributes) |attr| {
            ...
            _ = pthread_attr_init(&a);
        }
    } else {
        _ = pthread_attr_init(&a);
        _ = pthread_attr_setstacksize(&a, io_thread_stack_size);
        _ = pthread_attr_setguardsize(&a, 0);
    }
    ...
    if (pthread_create(&td, &a, &ioThreadFunc, @ptrCast(&args)) != 0) {
        _ = pthread_mutex_lock(&q.lock);
        unrefQueue(q);
        std.c._errno().* = @intFromEnum(linux.E.AGAIN);
...
fn lio_listio_impl(mode: c_int, cbs: [*]const ?*aiocb, cnt: c_int, sev: ?*sigevent_t) callconv(.c) c_int {
    ...
    if (st) |s| {
        ...
        if (sev.?.sigev_notify == SIGEV_THREAD) {
            ...
            _ = pthread_attr_init(&a);
            _ = pthread_attr_setstacksize(&a, std.heap.page_size_min);
            _ = pthread_attr_setguardsize(&a, 0);
        }
        _ = pthread_attr_setdetachstate(&a, PTHREAD_CREATE_DETACHED);
        var set: std.c.sigset_t = undefined;
        _ = std.c.sigfillset(&set);
        _ = pthread_sigmask(linux.SIG.BLOCK, &set, &set_old);
        if (pthread_create(&td, &a, &waitThread, @ptrCast(s)) != 0) {
            free(@ptrCast(s));
            std.c._errno().* = @intFromEnum(linux.E.AGAIN);
            return -1;
        }
...
fn aio_atfork_impl(who: c_int) callconv(.c) void {
    ...
    if (pthread_rwlock_tryrdlock(&maplock) != 0) {
        map = null;
        return;
    }
    if (map) |m| {
