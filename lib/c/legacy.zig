...
fn inetNtoaArg(raw: in_addr_t) in_addr_t {
    if (comptime builtin.cpu.arch == .aarch64_be) {
        const x0 = asm volatile ("mov %[ret], x0"
            : [ret] "=r" (-> usize),
        );
        return @truncate(x0 >> 32);
    }
    return raw;
}

fn inet_ntoa_impl(raw: in_addr_t) callconv(.c) [*]u8 {
    const in = inetNtoaArg(raw);
    const bytes: [4]u8 = switch (builtin.cpu.arch.endian()) {
        .little => .{
            @as(u8, @truncate(in)),
            @as(u8, @truncate(in >> 8)),
            @as(u8, @truncate(in >> 16)),
            @as(u8, @truncate(in >> 24)),
        },
        .big => .{
            @as(u8, @truncate(in >> 24)),
            @as(u8, @truncate(in >> 16)),
...
fn futimesLinux(fd: c_int, tv: ?*const [2]timeval) callconv(.c) c_int {
    if (tv) |t| {
        const times = [2]linux.timespec{
            .{ .sec = t[0].tv_sec, .nsec = t[0].tv_usec * 1000 },
            .{ .sec = t[1].tv_sec, .nsec = t[1].tv_usec * 1000 },
        };
...
fn lutimesLinux(path: [*:0]const u8, tv: ?*const [2]timeval) callconv(.c) c_int {
    if (tv) |t| {
        const times = [2]linux.timespec{
            .{ .sec = t[0].tv_sec, .nsec = t[0].tv_usec * 1000 },
            .{ .sec = t[1].tv_sec, .nsec = t[1].tv_usec * 1000 },
