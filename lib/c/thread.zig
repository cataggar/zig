...
const pthread_rwlock_impl = extern struct {
    ...
    _rw_waiters: c_int = 0,
    _rw_shared: c_int = 0,
    _padding: [rwlock_size - 3 * @sizeOf(c_int)]u8 = @splat(0),
};
const pthread_cond_impl = extern struct {
    _c_shared: usize = 0,
    _pad1: [16 - @sizeOf(usize)]u8 = @splat(0),
    _c_clock: c_int = 0,
    _pad2: [48 - 16 - @sizeOf(c_int)]u8 = @splat(0),
};
...
const pthread_barrier_impl = extern struct {
    ...
    _b_waiters: c_int = 0,
    _b_limit: c_int = 0,
    _padding: [barrier_size - 3 * @sizeOf(c_int)]u8 = @splat(0),
};
...
const sem_impl = extern struct {
    __val: [sem_val_len]c_int = @splat(0),
};
...
var ptc_rwlock: [rwlock_ints]c_int = @splat(0);
...
fn get_tp() usize {
    return switch (arch) {
        .x86_64 => if (@sizeOf(usize) == 8) asm volatile ("movq %%fs:0, %[ret]"
            : [ret] "=r" (-> usize),
        ) else asm volatile ("movl %%fs:0, %[ret]"
            : [ret] "=r" (-> usize),
        ),
        ...
        .riscv64, .riscv32 => asm volatile ("mv %[ret], tp"
            : [ret] "=r" (-> usize),
        ),
        .s390x => asm volatile (
            \\ear  %[ret], %%a0
...
fn pthread_getname_np_fn(thread: std.c.pthread_t, name: [*]u8, len: usize) callconv(.c) c_int {
    ...
    const path = std.fmt.bufPrintSentinel(&path_buf, "/proc/self/task/{d}/comm", .{tid}, 0) catch unreachable;
...
fn set_thread_area_fn(p: usize) callconv(.c) c_int {
    if (@hasField(linux.SYS, "set_thread_area")) {
        return @truncate(-@as(isize, @bitCast(linux.syscall1(linux.SYS.set_thread_area, p))));
    } else {
        return -eint(.NOSYS);
    }
}
