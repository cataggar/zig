...
const off_locale = part1_size + map_base_off + 11 * ptr_size + 8;
const LocaleStruct = extern struct { cat: [6]?*const anyopaque };
extern "c" fn pthread_self() *anyopaque;

/// Musl treats any non-null LC_CTYPE locale map as UTF-8 (CURRENT_UTF8).
fn currentUtf8() bool {
    if (!builtin.target.isMuslLibC()) return false;
    const self = @intFromPtr(pthread_self());
    const loc_ptr: *?*const LocaleStruct = @ptrFromInt(self + off_locale);
    const loc = loc_ptr.* orelse return false;
    return loc.cat[0] != null;
}
...
const bittab = [_]u32{
    ...
    C(0x8), C(0x9), C(0xa), C(0xb), C(0xc), C(0xd),
    C(0xe), C(0xf), D(0x0), D(0x1), D(0x2), D(0x3),
    D(0x4), D(0x5), D(0x6), D(0x7), D(0x8), D(0x9),
    D(0xa), D(0xb), D(0xc), D(0xd), D(0xe), D(0xf),
    E(0x0), E(0x1), E(0x2), E(0x3), E(0x4), E(0x5),
    E(0x6), E(0x7), E(0x8), E(0x9), E(0xa), E(0xb),
    E(0xc), E(0xd), E(0xe), E(0xf), F(0x0), F(0x1),
    F(0x2), F(0x3), F(0x4),
};
