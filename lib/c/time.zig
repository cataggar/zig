...
fn clockNanosleepSyscall() linux.SYS {
    return comptime if (@hasField(linux.SYS, "clock_nanosleep_time64"))
        @field(linux.SYS, "clock_nanosleep_time64")
    else
        @field(linux.SYS, "clock_nanosleep");
}
...
fn clockGettime(clk: linux.clockid_t, ts: *Timespec) c_int {
    var kts: linux.kernel_timespec = undefined;
    if (errno(linux.syscall2(clockGettimeSyscall(), @intFromEnum(clk), @intFromPtr(&kts))) < 0) return -1;
...
fn ftimeLinux(tp: *timeb) callconv(.c) c_int {
    ...
    if (clockGettime(.REALTIME, &ts) < 0) return -1;
    tp.time = ts.sec;
...
fn timespec_getLinux(ts: *Timespec, base: c_int) callconv(.c) c_int {
    ...
    if (clockGettime(.REALTIME, ts) < 0) return 0;
    return base;
}
...
fn nanosleepLinux(req: *const Timespec, rem: ?*Timespec) callconv(.c) c_int {
    const err = clockNanosleep(@intFromEnum(linux.CLOCK.REALTIME), 0, req, rem);
    if (err != 0) {
        std.c._errno().* = err;
        return -1;
    }
...
fn clockNanosleep(clk: c_int, flags: c_int, req: *const Timespec, rem: ?*Timespec) c_int {
    if (clk == @intFromEnum(linux.CLOCK.THREAD_CPUTIME_ID)) return @intFromEnum(linux.E.INVAL);

    var kreq = toKernelTimespec(req);
    var krem: linux.kernel_timespec = undefined;
    const rem_arg: usize = if (rem != null) @intFromPtr(&krem) else 0;
    ...
    const err = syscallError(ret);
    if (err != 0) {
        if (err == @intFromEnum(linux.E.INTR) and rem != null and (flags & 1) == 0)
            fromKernelTimespec(rem.?, krem);
        return err;
    }
    return 0;
}

fn clock_nanosleepLinux(clk: c_int, flags: c_int, req: *const Timespec, rem: ?*Timespec) callconv(.c) c_int {
    return clockNanosleep(clk, flags, req, rem);
}
