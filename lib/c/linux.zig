...
fn arg(val: anytype) linux.syscall_arg_t {
    const T = @TypeOf(val);
...
fn epoll_create1Linux(flags: c_int) callconv(.c) c_int {
    return errno(linux.epoll_create1(@bitCast(flags)));
}
...
fn mountLinux(special: ?[*:0]const u8, dir: [*:0]const u8, fstype: ?[*:0]const u8, flags: c_ulong, data: ?*const anyopaque) callconv(.c) c_int {
    return errno(linux.mount(special, dir, fstype, @intCast(flags), @intCast(arg(data))));
}
...
fn ptraceLinux(req: c_int, pid: linux.pid_t, addr: ?*anyopaque, data: ?*anyopaque) callconv(.c) c_long {
    ...
    const actual_addr: usize = if (addr) |ptr| @intFromPtr(ptr) else 0;
    const actual_data: usize = if (req_u -% 1 < 3) @intFromPtr(&result) else if (data) |ptr| @intFromPtr(ptr) else 0;
    const ret: isize = @bitCast(linux.ptrace(@intCast(req_u), pid, actual_addr, actual_data, 0));
    if (ret < 0) {
        @branchHint(.unlikely);
...
fn unshareLinux(flags: c_int) callconv(.c) c_int {
    return errno(linux.unshare(@intCast(@as(u32, @bitCast(flags)))));
}
