...
fn fexecveLinux(fd: c_int, argv: [*:null]const ?[*:0]c_char, envp: [*:null]const ?[*:0]c_char) callconv(.c) c_int {
    ...
    const proc_path = std.fmt.bufPrintSentinel(&buf, "/proc/self/fd/{d}", .{@as(u32, @intCast(fd))}, 0) catch unreachable;
...
test swab {
    ...
    @memset(a[0..], '\x00');
    swab("abcd", &a, 0);
    try std.testing.expectEqualSlices(u8, "\x00\x00\x00\x00", &a);
    swab("abcd", &a, -1);
    try std.testing.expectEqualSlices(u8, "\x00\x00\x00\x00", &a);
