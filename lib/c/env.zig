...
fn __init_ssp(entropy: ?*const anyopaque) callconv(.c) void {
    if (entropy) |ent| {
        const src: [*]const u8 = @ptrCast(ent);
        @memcpy(std.mem.asBytes(&__stack_chk_guard), src[0..@sizeOf(usize)]);
    } else {
...
fn __init_libc_fn(envp: [*:null]?[*:0]u8, pn: ?[*:0]u8) callconv(.c) void {
    ...
    const SYS_ppoll = if (@hasField(linux.SYS, "ppoll")) linux.SYS.ppoll else linux.SYS.ppoll_time64;
...
const MainFn = *const fn (c_int, [*]?[*:0]u8, [*:null]?[*:0]u8) callconv(.c) c_int;

fn libc_start_main_stage2(
    main_fn: MainFn,
    argc: c_int,
    argv: [*]?[*:0]u8,
    _: ?*anyopaque,
...
fn __libc_start_main_fn(
    ...
    const envp: [*:null]?[*:0]u8 = @ptrCast(argv + @as(usize, @intCast(argc)) + 1);
    @call(.never_inline, __init_libc_fn, .{ envp, argv[0] });
    return @call(.never_inline, libc_start_main_stage2, .{ main_fn, argc, argv, null, null, null });
}
