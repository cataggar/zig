...
fn roundToEven(comptime T: type, x: T) T {
    ...
    const ax = @abs(x);
    const max_exact: T = switch (T) {
        f32 => 0x1p23,
        f64 => 0x1p52,
        else => @compileError("unsupported float type"),
    };
    if (ax >= max_exact) return x;
    ...
    if (frac > 0.5) {
        ...
        const i: u64 = @intFromFloat(lo);
        if ((i & 1) != 0) rounded = lo + 1.0;
    }
    return std.math.copysign(rounded, x);
}

fn nearbyintfImpl(x: f32) callconv(.c) f32 {
    return roundToEven(f32, x);
}
fn nearbyintImpl(x: f64) callconv(.c) f64 {
    return roundToEven(f64, x);
}
...
// =========================================================================
fn openatImpl(dirfd: c_int, pathname: [*:0]const u8, flags: c_int, mode: c_int) callconv(.c) c_int {
    _ = mode;
...
// =========================================================================
var __wasilibc_environ: ?[*:null]align(1) ?[*:0]u8 = @ptrFromInt(@as(usize, @bitCast(@as(isize, -1))));
var empty_environ: [1]?[*:0]u8 = .{null};

fn wasilibcEnsureEnvironImpl() callconv(.c) void {
    if (@intFromPtr(__wasilibc_environ) == @as(usize, @bitCast(@as(isize, -1)))) {
        wasilibcInitializeEnvironImpl();
    }
}

fn wasilibcGetEnvironImpl() callconv(.c) ?[*:null]align(1) ?[*:0]u8 {
...
fn wasilibcInitializeEnvironImpl() callconv(.c) void {
    ...
    if (environ_count == 0) {
        __wasilibc_environ = @ptrCast(&empty_environ);
        return;
    }
...
fn wasilibcDeinitializeEnvironImpl() callconv(.c) void {
    if (@intFromPtr(__wasilibc_environ) != @as(usize, @bitCast(@as(isize, -1)))) {
        _ = clearenv();
        __wasilibc_environ = @ptrFromInt(@as(usize, @bitCast(@as(isize, -1))));
    }
}
...
comptime {
    if (builtin.link_libc) {
        ...
        // Exported globals
        @export(&__wasilibc_environ, .{ .name = "__wasilibc_environ", .linkage = .weak, .visibility = .hidden });
        @export(&__wasilibc_environ, .{ .name = "_environ", .linkage = .weak, .visibility = .hidden });
        @export(&__wasilibc_environ, .{ .name = "environ", .linkage = .weak, .visibility = .hidden });
        @export(&wasilibc_cwd, .{ .name = "__wasilibc_cwd", .linkage = .weak, .visibility = .hidden });
    }
}
